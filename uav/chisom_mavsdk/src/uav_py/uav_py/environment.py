...
class Environment(object):
    def __init__(self):
        ...
        # Setup control and vision modules
        self.setup_control_module()  # Call this during initialization
        ...
        self.action_space = [
            [x, y, z]
            for x in x_action_space
            for y in y_action_space
            for z in z_action_space
        ]
    ...
    def setup_control_module(self):
        try:
            ...
            shm = SharedMemory(
                name=SHM_OBJECT_NAME,
                flags=posix_ipc.O_CREX,  # Create the shared memory
                mode=0o644,
                size=ctypes.sizeof(V2C_Memory)
            )
        
        # Map the shared memory object to a memory-mapped file
        mm = mmap(shm.fd, shm.size)
        self.v2c = V2C_Memory.from_buffer(mm)

    # ... Rest of your functions (setup_vision_module, step, reset, etc.)
